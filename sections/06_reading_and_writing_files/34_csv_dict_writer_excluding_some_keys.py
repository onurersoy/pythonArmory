...
# Let's delete 'rank' key from the below list:
# columns = ['country', 'gold', 'silver', 'bronze', 'rank']
columns = ['country', 'gold', 'silver', 'bronze']
# You might think that's all you need to do, but it gives an error. Run the program: We get a ValueError, the dict
# contains fields not in fieldnames. It also shows the missing field name – "rank", in this case. The default
# behaviour is to raise an error, which is what happened here. But we can tell the DictWriter to ignore the error
# instead. Add an 'extrasaction' argument on line 115:
...
# The "rank" field has been excluded. We have the flexibility to only write the columns we want, but we have to
# explicitly tell the DictWriter that we wanted to do that. The default behaviour, when a key is missing from the
# fieldnames list, is to raise an error.

# The DictWriter is very flexible. If some of the dictionaries are missing a key, we can also specify what the
# DictWriter should do. Check out the 'restval' parameter in the documentation if that's something you'll need to do.
