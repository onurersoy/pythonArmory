...
# A null-terminated string is a sequence of characters that ends with at least one zero byte. They're used extensively
# in the C programming language, and for that reason they're also referred to as c-strings. When reading
# a null-terminated string, we keep reading byte-by-byte, until we encounter a byte whose value is zero. When that
# happens, we've reached the end of the string. An empty string will consist of just a zero byte.
