...
# We've seen that the 'DictWriter' will need dictionaries to write to the CSV file.
...
with open(filename, 'w', encoding='utf-8', newline='') as output_file:
    ...
    writer.writeheader()  # If you don't call the writeheader method, you won't get the headings in the CSV file.

    # There are two ways you can use a DictWriter to write the data. You can either iterate over your sequence of
    # dictionaries, and write the data row by row, or you can send the entire sequence to the writer. Let's write
    # each row separately, first:
    for row in medals_table:
        writer.writerow(row)
        # Writing row by row would be useful if you wanted to perform some operation on the data, while iterating
        # over it.

    # If you just want to dump the data into a CSV file, you can let the writer do the iterating. So, comment out
    # the 'for loop' above, then use the 'writerows' method instead:
    writer.writerows(medals_table)

    # Let's also sort the data:
    # writer.writerows(sorted(medals_table))
    # We've got a TypeError^^. When you sort things, the sorting function has to compare them, to decide what order
    # they go in. The less than operator isn't defined for dictionaries – how would you decide if one dictionary is
    # less than or greater than another? Because they can't be compared, they can't be sorted. What we have to do is
    # create a small function that returns the value that we want to sort on. The sorted function will then call that
    # function to get the values that can be compared. Let's create the 'sort_key' function above...

    # Now let's re-try sorting:
    writer.writerows(sorted(medals_table, key=sort_key))
    # Be careful when passing a function as an argument^^. Your IDE will normally expect you to be calling the
    # function, and will often add the opening and closing parentheses automatically. We're not calling the function
    # here. We're providing the name of the function, and the sorted function will call it when it needs it. So we
    # want the function name only, without the opening and closing parentheses.

# PS. Dictionaries can be sorted by the sorted function, but you have to create another function; that returns the key
# to sort on.
